...
本系统的安全执行框架遵循\textbf{默认拒绝（Default Deny）}原则，
...
命令安全模块（\texttt{src/command\_safety.py}）的核心数据结构为\texttt{SafetyResult}，
包含四个字段：
...
\texttt{classify\_command()}函数按以下优先级顺序判定风险等级，
一旦命中立即返回，不继续匹配后续规则：

\textbf{第一级——字符串黑名单（blocked）}：
...
所有判定结果均可通过运行
\texttt{src/command\_safety.py}中的\texttt{classify\_command()}函数复现。

\begin{table}[htbp]
\centering
\caption{典型命令安全分级样例}
\label{tab:safety-samples}
\begin{tabular}{p{5.5cm}p{4.2cm}p{2.8cm}}
\toprule
命令示例 & 命中规则级别 & 判定结果 \\
\midrule
...
七级规则链在设计时面临\textbf{误拦截（False Positive）}与\textbf{漏拦截（False Negative）}
...
每次用户确认执行命令前，CLI客户端触发\texttt{\_run\_security\_checks()}流水线，
依次执行三个校验步骤，如图~\ref{fig:pipeline}~所示。

\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2.2cm}图占位符：预执行三步校验流水线（fig:pipeline）\vspace{2.2cm}}}
\caption{预执行三步校验流水线}
\label{fig:pipeline}
\end{figure}
...
\textbf{步骤三——效果模拟（\texttt{simulate\_command()}）}：
...
执行日志模块（\texttt{src/execution\_logger.py}）以JSONL格式追加写入
\texttt{data/execution\_log.jsonl}，每行一条JSON记录。
